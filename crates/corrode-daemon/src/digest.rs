...
/// Bytes of raw output kept when nothing is recognized.
const FALLBACK_CAP: usize = 4096;
...
fn head_tail(s: &str, cap: usize) -> String {
    ...
    let head_len = crate::tools::floor_char_boundary(s, cap * 2 / 3);
    let tail_start = s.len() - (cap / 3);
